...
# siRNA synthetic lethality screen for the 51 genes that associate with only AKT3.
...
```{r}
R_genes <- as.data.frame(AKT3_final)
```

Match the 51 genes with their SL ratio score from the siRNA test and retrieve only the relevant data.

```{r}
geneCoords <- match(as.vector(R_genes[,1]),as.vector(allGns[,1]))
R_genes_matched <- allGns[geneCoords,]
R_genes_matched %>% dim()
#write.csv(R_genes_matched, file = "siRNA51.csv")
```

Plot

```{r}
pdf("siRNA51.pdf", height=4)
x<-allGns
y<-R_genes_matched
...
plot(density(x[,4]),col='blue',xlim=c(mn,mx), ylim=c(0,max(c(d1,d2))),
     xlab="Cell viability ratio of the 51 AKT3-associated genes (FC)",ylab="Density")
# Add legend
legend(1.35,1.8,c("All", "51 Genes"),fill=c(4,2), title=kst,cex=0.8)
...
d2max<-density(y[1:51,4])
d2max$x[which.max(d2max$y)]
# Non sensory GPCRs left (SL) peak: 0.9751709
dev.off()
```