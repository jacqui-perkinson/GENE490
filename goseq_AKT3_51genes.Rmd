...
# 51 genes associating with AKT3.
...
Make 51 genes 1, and total genes 0.

```{r}
x <- as.integer(ifelse(totalgns %in% AKT3_only, 1, 0))
names(x) <- totalgns
table(x)
class(x)
```
...
goterms <- nobias$category  ##[p.adjust(nobias$over_represented_pvalue)<0.05]
...
#write.csv(over, file = "GO_51_over.csv")
```
