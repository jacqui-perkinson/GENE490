...
# 3498 genes associating with AKT1.
...
```{r}
x <- as.integer(ifelse(totalgns %in% AKT3_only_allsig, 1, 0))
names(x) <- totalgns
table(x)
class(x)
```
...
```{r}
#pdf("goseqbias3498.pdf", height=4)
...
#write.csv(over, file = "GO_3298_over.csv")
```
