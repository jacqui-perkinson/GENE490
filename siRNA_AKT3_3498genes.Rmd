...
# siRNA synthetic lethality screen for the 3498 genes that associate with only AKT3.
...
```{r}
R_genes <- as.data.frame(AKT3_allsig)
```

Match the 3498 genes with their SL ratio score from the siRNA test and retrieve only the relevant data.
...
R_genes_matched %>% dim()
#write.csv(R_genes_matched, file = "siRNA3498.csv")
```

Plot

```{r}
pdf("siRNA3498.pdf", height=4)
x<-allGns
y<-R_genes_matched
...
plot(density(x[,4]),col='blue',xlim=c(mn,mx), ylim=c(0,max(c(d1,d2))),
     xlab="Cell viability ratio of the 3498 AKT3-associated genes",ylab="Density")
# Add legend
legend(1.25,1.8,c("All", "3498 Genes"),fill=c(4,2), title=kst,cex=0.8)
...
d2max<-density(y[1:3355,4])
d2max$x[which.max(d2max$y)]
# Non sensory GPCRs left (SL) peak: 0.9959945
dev.off()
```
